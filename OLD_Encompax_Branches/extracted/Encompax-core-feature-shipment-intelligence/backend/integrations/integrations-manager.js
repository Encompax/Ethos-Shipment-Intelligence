 ...
 * 
 * Integrations orchestrator — starts/manages all active capability workers.
 * Vendor-specific logic lives in adapter modules referenced by the registry.
 * 
...
const { INTEGRATION_REGISTRY, loadIntegration } = require('../config/integrations');
 ...
 * @param {string} integrationId - capability id such as 'hris' or 'erp'
 */
async function startIntegration(integrationId) {
  ...
  try {
    const WorkerClass = loadIntegration(integrationId);
